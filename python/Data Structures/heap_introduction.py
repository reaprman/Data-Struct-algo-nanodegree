...
class Heap:
    ...
    def insert(self, data):
        ...
        if self.next_index >= len(self.cbt):
            tmp = self.cbt
            self.cbt = [None for _ in range(len(tmp)*2)]
            for index in range(len(tmp)-1):
                self.cbt[index] = tmp[index]
        pass
    
    def _up_heapify(self):
        current_index = self.next_index
        while current_index >= 1:
            current_value = self.cbt[current_index]
            parent_index = (current_index-1)//2
            parent_value = self.cbt[parent_index]
            if current_value < parent_value:
                self.cbt[current_index] = parent_value
                self.cbt[parent_index] = current_value
    ...
    def remove(self):
        ...
        first_value = self.cbt[first]
        last_value = self.cbt[last]
        
        self.cbt[first] = last_value
        self.cbt[last] = None
        self.next_index -= 1
    ...
    def _down_heapify(self):
        ...
        while parent_index < self.next_index:
            ...
            parent_value = self.cbt[parent_index]
            if child_index1 < len(self.cbt):
                child_value1 = self.cbt[child_index1]
            if child_index2 < len(self.cbt)
            child_value2 = self.cbt[child_index2]
            
            if child_value1 < child_value2:
                lowest_index = child_index1
                lowest_value = child_value1
            elif child_value1 > child_value2:
                lowest_index = child_index2
                lowest_value = child_value2
            
            if parent_value > lowest_value:
                self.cbt[parent_index] = lowest_value
                self.cbt[lowest_index] = parent_value
                parent_index = lowest_index
            else:
                break
        pass
...
class Heap:
    ...
    def insert(self, data):
        ...
        #if the next_index is larger than array copy elements into array twice as large
        if self.next_index >= len(self.cbt):
            temp = self.cbt
    ...
    def _up_heapify(self):
        ...
        while current_index >= 1:
            ...
            if current_value < parent_value:
                ...
                self.cbt[parent_index] = current_value
                current_index = parent_index
            # if else statement not used with break line, unnecessary runs of while loop will occur
            # if values cant be swapped then then no longer need to heapify
            else:  
                break 
...
class Heap:
    ...
    def _down_heapify(self):
        ...
        while parent_index < self.next_index:
            ...
            parent_value = self.cbt[parent_index]
            if child_index1 < self.next_index:
                child_value1 = self.cbt[child_index1]
            if child_index2 < self.next_index:
                child_value2 = self.cbt[child]
    ...
    def remove(self):
        ...
        front_value = self.cbt[front]
        end_value = self.cbt[end]

        self.cbt[front] = end_value
        self.cbt[end] = None
        self.next_index -= 1
